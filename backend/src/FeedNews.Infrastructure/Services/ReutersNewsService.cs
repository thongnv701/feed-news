...
public class ReutersNewsService : IReutersNewsService
{
    private readonly IHttpClientFactory _httpClientFactory;
    private readonly NewsFeedsConfiguration _feedsConfig;
    private readonly ILogger<ReutersNewsService> _logger;

    public ReutersNewsService(
        IHttpClientFactory httpClientFactory,
        IOptions<NewsFeedsConfiguration> feedsConfig,
        ILogger<ReutersNewsService> logger)
    {
    ...
    {
        _logger.LogInformation("Fetching Reuters news for category: {Category}", category);
        
        var categoryName = category.ToString();
        
        if (!_feedsConfig.Reuters.RssFeedUrls.ContainsKey(categoryName))
        {
            _logger.LogWarning("No Reuters RSS URL configured for category: {Category}", categoryName);
            return new List<News>();
        }

        var rssUrl = _feedsConfig.Reuters.RssFeedUrls[categoryName];

        try
        {
            var httpClient = _httpClientFactory.CreateClient("ReutersClient");
            ...
            var policy = Policy
                ...
                .WaitAndRetryAsync(
                    ...
                    {
                        _logger.LogWarning(
                            "Retry {RetryCount} for Reuters feed after {DelayMs}ms. Exception: {Exception}",
                            retryCount,
            ...
            var newsList = ParseRssFeed(content, NewsSource.Reuters, category);

            _logger.LogInformation("Successfully fetched {Count} articles from Reuters for category {Category}",
        ...
        {
            _logger.LogError(ex, "Error fetching Reuters news for category {Category}", categoryName);
            return new List<News>();
        }
    }
