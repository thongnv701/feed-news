...
public class PaginationResponse<TEntity> : PaginationResponse<TEntity, TEntity> where TEntity : class
{
    ...
    public PaginationResponse(IQueryable<TEntity> source, int pageNumber, int pageSize) : base(source, pageNumber,
        pageSize, entity => entity)
    {
    }
}